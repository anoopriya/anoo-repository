...
class ApplicationSpec extends WordSpec with Matchers {
  ...
  "Application Controller" should {
    ...
    "render the index page" in withPlay() {
      val index = route(application(), FakeRequest(GET, "/v1/hello-world-service")).get

      status(index) shouldBe OK
      contentType(index).get == "application/json" shouldBe true
      (contentAsJson(index) \ "response" \ "results").as[String] == "hello-world-service is up and running!" shouldBe true
    }
    ...
    "change the log Level" in withPlay() {
      val changeLog = route(application(), FakeRequest(GET, "/v1/hello-world-service/logLevel/WARN")).get
      ...
      Await.result(route(application(), FakeRequest(GET, "/v1/hello-world-service/logLevel/DEBUG")).get, 10 seconds)
      Logger.isDebugEnabled shouldBe true
    }

    "not process incorrect log Level" in withPlay() {
      val result = route(application(), FakeRequest(GET, "/v1/hello-world-service/logLevel/WARN2")).get
      status(result) shouldBe NOT_FOUND
    }
  }
}
